...
const SCRIPT_URL = "https://script.google.com/macros/s/AKfycbxapDxmxFFnvZcwjopuPlC6Y6mXsfukdj5zlDP58-4R5L2glE1eJTutwnrEqNsrzOhK/exec"; 
...
// --- LOGIKA KASIR, LAPORAN & STRUK (DENGAN PERBAIKAN) ---
...
function pilihBarang(item) {
    ...
    namaBarangTerpilihSpan.innerHTML = `${item.Nama_Barang} <small>(Stok: ${item.Stok_Dasar})</small>`;
    selectSatuanKasir.innerHTML = '';
    selectSatuanKasir.add(new Option(`Pcs / Kg - ${formatRupiah(item.Harga_Dasar)}`, 'Dasar'));
    if (item.Harga_Paket > 0 && item.Konversi_Paket > 0) {
        selectSatuanKasir.add(new Option(`Paket - ${formatRupiah(item.Harga_Paket)}`, 'Paket'));
    }
    if (item.Harga_Karton > 0 && item.Konversi_Karton > 0) {
...
function handleTambahKeKeranjang(e) {
    ...
    const dasarDiKeranjang = keranjang.filter(item => item.idBarang === itemData.ID_Barang).reduce((total, item) => total + item.jumlahDasar, 0);
    let dasarAkanDitambah = 0;
    if (satuanDiminta === 'Dasar') { dasarAkanDitambah = jumlahDiminta; }
    else if (satuanDiminta === 'Paket') { dasarAkanDitambah = jumlahDiminta * itemData.Konversi_Paket; }
    else if (satuanDiminta === 'Karton') { dasarAkanDitambah = jumlahDiminta * itemData.Konversi_Karton; }
    if ((dasarDiKeranjang + dasarAkanDitambah) > itemData.Stok_Dasar) {
        const sisaStokEfektif = itemData.Stok_Dasar - dasarDiKeranjang;
        alert(`Stok tidak mencukupi!\n\nStok Awal: ${itemData.Stok_Dasar}\nSudah di Keranjang: ${dasarDiKeranjang}\nSisa Stok Tersedia: ${sisaStokEfektif}`);
        return;
    }
    let hargaSatuan = 0;
    if (satuanDiminta === 'Dasar') { hargaSatuan = itemData.Harga_Dasar; }
    else if (satuanDiminta === 'Paket') { hargaSatuan = itemData.Harga_Paket; }
    else if (satuanDiminta === 'Karton') { hargaSatuan = itemData.Harga_Karton; }
    const itemDiKeranjang = {
        idBarang: itemData.ID_Barang, namaBarang: itemData.Nama_Barang, jumlah: jumlahDiminta, jumlahDasar: dasarAkanDitambah, satuan: satuanDiminta,
        hargaSatuan: hargaSatuan, subtotal: jumlahDiminta * hargaSatuan,
        dataAsli: { Harga_Dasar: itemData.Harga_Dasar, Konversi_Paket: itemData.Konversi_Paket, Harga_Paket: itemData.Harga_Paket, Konversi_Karton: itemData.Konversi_Karton, Harga_Karton: itemData.Harga_Karton }
    };
    ...
    if (indexAda > -1) {
        ...
        keranjang[indexAda].jumlahDasar += dasarAkanDitambah;
...
function updateKuantitasKeranjang(index, jumlahBaru) {
    ...
    if (!itemDataAsliServer) { alert('Data barang tidak ditemukan. Coba muat ulang halaman.'); renderKeranjang(); return; }
    const stokAwal = itemDataAsliServer.Stok_Dasar;
    const dasarLainDiKeranjang = keranjang.filter((_, i) => i !== parseInt(index) && keranjang[i].idBarang === item.idBarang).reduce((total, itemLain) => total + itemLain.jumlahDasar, 0);
    let dasarDimintaSekarang = 0;
    if (item.satuan === 'Dasar') { dasarDimintaSekarang = jumlahBaru; }
    else if (item.satuan === 'Paket') { dasarDimintaSekarang = jumlahBaru * item.dataAsli.Konversi_Paket; }
    else if (item.satuan === 'Karton') { dasarDimintaSekarang = jumlahBaru * item.dataAsli.Konversi_Karton; }
    if ((dasarLainDiKeranjang + dasarDimintaSekarang) > stokAwal) {
        const sisaStokEfektif = stokAwal - dasarLainDiKeranjang;
        alert(`Stok tidak mencukupi!\n\nStok Awal: ${stokAwal}\nItem lain di Keranjang: ${dasarLainDiKeranjang}\nSisa Stok Tersedia: ${sisaStokEfektif}`);
        renderKeranjang();
        return;
    }
    item.jumlah = jumlahBaru;
    item.jumlahDasar = dasarDimintaSekarang;
...
function updateSatuanKeranjang(index, satuanBaru) {
    ...
    if (!item) return;
    const itemDataAsliServer = semuaDataBarang.find(i => i.ID_Barang === item.idBarang);
    if (!itemDataAsliServer) { alert('Data barang tidak ditemukan. Coba muat ulang halaman.'); return; }
    let hargaSatuanBaru = 0; let jumlahDasarBaru = 0;
    if (satuanBaru === 'Dasar') { hargaSatuanBaru = itemDataAsliServer.Harga_Dasar; jumlahDasarBaru = item.jumlah; }
    else if (satuanBaru === 'Paket') { hargaSatuanBaru = itemDataAsliServer.Harga_Paket; jumlahDasarBaru = item.jumlah * itemDataAsliServer.Konversi_Paket; }
    else if (satuanBaru === 'Karton') { hargaSatuanBaru = itemDataAsliServer.Harga_Karton; jumlahDasarBaru = item.jumlah * itemDataAsliServer.Konversi_Karton; }
    const stokAwal = itemDataAsliServer.Stok_Dasar;
    const dasarLainDiKeranjang = keranjang.filter((_, i) => i !== parseInt(index) && keranjang[i].idBarang === item.idBarang).reduce((total, itemLain) => total + itemLain.jumlahDasar, 0);
    if ((dasarLainDiKeranjang + jumlahDasarBaru) > stokAwal) {
    ...
    item.jumlahDasar = jumlahDasarBaru;
    item.subtotal = item.jumlah * hargaSatuanBaru;
    item.dataAsli = { Harga_Dasar: itemDataAsliServer.Harga_Dasar, Konversi_Paket: itemDataAsliServer.Konversi_Paket, Harga_Paket: itemDataAsliServer.Harga_Paket, Konversi_Karton: itemDataAsliServer.Konversi_Karton, Harga_Karton: itemDataAsliServer.Harga_Karton };
    renderKeranjang();
}
